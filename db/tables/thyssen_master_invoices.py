...
class ThyssenMasterInvoices(Base):
    """Table for storing Thyssen document processing records."""

    __tablename__ = "thyssen_master_invoices"
...
def list_thyssen_master_invoices(
    db: Session,
    limit: int = 50,
    offset: int = 0,
    document_type: Optional[str] = None,
    document_connector_id: Optional[str] = None,
) -> List[ThyssenMasterInvoices]:
    """List Thyssen documents with optional filtering."""
    query = db.query(ThyssenMasterInvoices)

    if document_type:
        query = query.filter(ThyssenMasterInvoices.document_type == document_type)

    if document_connector_id:
        query = query.filter(ThyssenMasterInvoices.document_connector_id == document_connector_id)

    return query.order_by(ThyssenMasterInvoices.id.desc()).offset(offset).limit(limit).all()


def get_thyssen_master_invoice(db: Session, document_id: int) -> Optional[ThyssenMasterInvoices]:
    """Get a specific Thyssen document by ID."""
    return db.query(ThyssenMasterInvoices).filter(ThyssenMasterInvoices.id == document_id).first()


def get_thyssen_master_invoice_by_uuid(
    db: Session, document_uuid: str, document_type: Optional[str] = None
) -> Optional[ThyssenMasterInvoices]:
    """Get a Thyssen document by UUID and optionally type."""
    query = db.query(ThyssenMasterInvoices).filter(ThyssenMasterInvoices.document_uuid == document_uuid)
    if document_type:
        query = query.filter(ThyssenMasterInvoices.document_type == document_type)
    return query.first()
...
def find_bl_number_by_reference(db: Session, reference: str) -> Optional[str]:
    ...
    invoice_doc = (
        db.query(ThyssenMasterInvoices)
        .filter(ThyssenMasterInvoices.document_uuid == reference, ThyssenMasterInvoices.document_type == "invoice")
        .first()
    )
    ...
    master_bill = (
        db.query(ThyssenMasterInvoices)
        .filter(ThyssenMasterInvoices.document_uuid == reference, ThyssenMasterInvoices.document_type == "master_bill")
        .first()
    )
...
def create_thyssen_master_invoice(
    db: Session,
    document_type: str,
    document_uuid: str,
...
) -> ThyssenMasterInvoices:
    """Create a new Thyssen document record."""
    document = ThyssenMasterInvoices(
...
def update_thyssen_master_invoice(
    db: Session,
    document_id: int,
    document_data: Optional[dict] = None,
    document_metadata: Optional[dict] = None,
    ...
    document = db.query(ThyssenMasterInvoices).filter(ThyssenMasterInvoices.id == document_id).first()
    if document:
...
def get_original_filename(document: ThyssenMasterInvoices) -> Optional[str]:
...
def set_processing_status(db: Session, document_id: int, status: str) -> Optional[ThyssenMasterInvoices]:
    """Convenience function to update just the processing status."""
    return update_thyssen_master_invoice(db, document_id, document_metadata={"processing_status": status})
